...
# 关键: 强制Matplotlib使用非交互式后端, 避免在服务器或无GUI环境中报错
try:
    matplotlib.use('Agg')
...
print("\n--- Model Testing Script (Enhanced Analysis Version) ---")
print("For each test image, this script will test all 4 rotations.")

# --- 1. Parameter Setup ---
MODEL_PATH = './model/mydata_rotnet_mish.pth' 
...
try:
    ...
    print(f"Error: Model file not found at '{MODEL_PATH}'. Please check the path and model definition. Error: {e}")
    exit()
...
# --- 5. Iterate Through Test Folder, Predict, and Analyze ---
...
angle_stats = {angle: [0, 0] for angle in angles} # 格式: {角度: [正确数, 总数]}

# 外层循环：遍历每个原始图片
for image_name in sorted(test_images):
    image_path = os.path.join(TEST_IMAGE_DIR, image_name)
    try:
        # 使用Pillow打开图片并校正EXIF方向
